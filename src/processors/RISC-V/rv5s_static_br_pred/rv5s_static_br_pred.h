...
public:
    ...
    /*RV5S_STATIC_BR_PRED() : RipesProcessor("5-Stage RISC-V Processor with Static Branch Prediction") {
        ...
        idex_br_or->out >> pc_control->br_decision;
        ...
        // idex_clear_or->out >> idex_reg->clear;
        ...
        br_and->out >> *idex_br_or->in[0];
        br_flag_and->out >> *idex_br_or->in[1];
        idex_br_or->out >> *pred_xor->in[0];
        idex_reg->btt_valid_out >> *pred_xor->in[1];
        pred_xor->out >> *idex_clear_or->in[0];
        ecallChecker->syscallExit >> *idex_clear_or->in[1];
        ...
        idex_clear_and->out >> *ifid_clear_or->in[0];
        // control->do_jump >> *ifid_clear_or->in[1];
        ifid_clear_and->out >> *ifid_clear_or->in[1];
    ...
    SUBCOMPONENT(idex_br_or, TYPE(Or<1, 2>));
    SUBCOMPONENT(pred_xor, TYPE(Xor<1, 2>));
    SUBCOMPONENT(idex_clear_or, TYPE(Or<1, 2>));
    SUBCOMPONENT(idex_clear_and, TYPE(And<1, 2>));
    SUBCOMPONENT(ifid_clear_and, TYPE(And<1, 2>));
    SUBCOMPONENT(idex_clear, TYPE(Or<1, 2>));
